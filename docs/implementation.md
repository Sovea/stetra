...
Without selectors or selected IDs or paths, context also returns `library.activeCount`, counting valid, active project-scoped knowledge without supplying its bodies. This distinguishes an empty selection from an empty library; it does not judge relevance or infer the task. Invalid files remain visible through `knowledge.issues`. Selector-based retrieval omits the count. An empty-library check creates no knowledge, index, or session cache.
