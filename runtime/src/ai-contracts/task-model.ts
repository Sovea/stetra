...
export function prepareTaskModelContract(input: TaskModelContractInput): TaskModelContractOutput {
  ...
  return {
    ...
    contract: {
      ...
      cacheKeyMaterial: { task: input.task, schemaId: 'runtime.task-model' },
    },
  };
}

export function validateTaskModelPayload(raw: unknown): TaskModelValidationResult {
  const versionDiagnostic = contractVersionDiagnostic(raw, 'task-model');
  if (versionDiagnostic) {
    return { models: [], diagnostics: buildContractPayloadDiagnostics('task-model', [versionDiagnostic]) };
  }
  const values = raw == null ? [] : Array.isArray(raw) ? raw : [raw];
  const entries: ContractPayloadDiagnosticEntry[] = [];
  const models: TaskModelProposal[] = [];
  ...
  return { models, diagnostics: buildContractPayloadDiagnostics('task-model', entries) };
}

function isTaskModelProposal(value: unknown): value is TaskModelProposal {
...
function firstInvalidField(model: TaskModelProposal): { field: string; reason: ContractPayloadDiagnosticEntry['reason']; message: string; confidence?: number } | null {
  const fields: Array<[string, unknown, readonly string[] | null, 'scalar' | 'list']> = [
    ...
    ['intent.changed_files', model.intent.changed_files, null, 'list'],
    ['intent.tech_stack', model.intent.tech_stack, null, 'list'],
    ['intent.tags', model.intent.tags, null, 'list'],
    ['context.project_stage', model.context.project_stage, TASK_INTERPRETATION_ENUMS.context.project_stage, 'scalar'],
    ['context.optimization_target', model.context.optimization_target, TASK_INTERPRETATION_ENUMS.context.optimization_target, 'scalar'],
    ['context.hard_constraints', model.context.hard_constraints, null, 'list'],
    ['context.allowed_tradeoffs', model.context.allowed_tradeoffs, null, 'list'],
    ['context.avoid', model.context.avoid, null, 'list'],
    ['context.risk_level', model.context.risk_level, TASK_INTERPRETATION_ENUMS.context.risk_level, 'scalar'],
    ['context.scope_size', model.context.scope_size, TASK_INTERPRETATION_ENUMS.context.scope_size, 'scalar'],
    ['context.compatibility_requirement', model.context.compatibility_requirement, TASK_INTERPRETATION_ENUMS.context.compatibility_requirement, 'scalar'],
    ['context.interface_sensitivity', model.context.interface_sensitivity, TASK_INTERPRETATION_ENUMS.context.interface_sensitivity, 'scalar'],
    ['context.refactor_tolerance', model.context.refactor_tolerance, TASK_INTERPRETATION_ENUMS.context.refactor_tolerance, 'scalar'],
    ['context.migration_phase', model.context.migration_phase, TASK_INTERPRETATION_ENUMS.context.migration_phase, 'scalar'],
    ['context.review_goal', model.context.review_goal, TASK_INTERPRETATION_ENUMS.context.review_goal, 'scalar'],
  ];

  for (const [field, candidate, allowedValues, kind] of fields) {
    if (candidate === undefined) continue;
    if (!isRecord(candidate)) return { field, reason: 'malformed-payload', message: 'Task model field must be an object.' };
    if (!validConfidence(candidate.confidence)) return { field, reason: 'malformed-payload', message: 'Task model field confidence must be between 0 and 1.' };
    if (candidate.confidence < 0.5) return { field, reason: 'low-confidence', message: 'Task model field confidence is below threshold.', confidence: candidate.confidence };
    if (!validEvidenceRefs(candidate.evidence_refs)) return { field, reason: 'missing-evidence', message: 'Task model field must include at least one valid evidence_ref.', confidence: candidate.confidence };
    if (kind === 'scalar') {
      if (typeof candidate.value !== 'string') return { field, reason: 'missing-required-field', message: 'Task model scalar field must include value.', confidence: candidate.confidence };
      if (allowedValues && !allowedValues.includes(candidate.value)) return { field, reason: 'unsupported-value', message: `Unsupported value "${candidate.value}".`, confidence: candidate.confidence };
    } else {
      if (!Array.isArray(candidate.values) || !candidate.values.every((item) => typeof item === 'string')) return { field, reason: 'missing-required-field', message: 'Task model list field must include string values.', confidence: candidate.confidence };
    }
  }
  return null;
}

function buildTaskModelPrompt(input: TaskModelContractInput): string {
  return [
