...
function resolveGraphExecutionIntent(
  directive: DirectiveIR,
  relations: SemanticRelationIR[],
): DirectiveDecision | null {
  const candidates = relations
    .filter((relation) =>
      relation.adjudication.status === 'accepted'
      && relation.impact === 'execution-mode'
      && relation.executionIntent != null
      && relation.executionIntent !== 'no-change')
    .sort((left, right) => executionIntentRank(right.executionIntent) - executionIntentRank(left.executionIntent));
...
function resolveGraphExecutionIntentCandidate(
  directive: DirectiveIR,
  selected: SemanticRelationIR,
  relations: SemanticRelationIR[],
): DirectiveDecision | null {
...
function executionIntentRank(intent: SemanticRelationIR['executionIntent']): number {
  switch (intent) {
    case 'suppress':
      return 5;
    case 'deviation-noted':
      return 4;
    case 'enforce':
      return 3;
    case 'ambient':
