...
//#region src/ai-contracts/adherence-evaluation.ts
...
const VALID_IGNORED_REASONS = new Set([
	"not-applicable",
	"conflicts-with-task",
	"too-broad",
	"repo-reality",
...
function prepareAdherenceEvaluationContract(input) {
	...
	return {
		...
		contract: {
			...
			allowedIds: { directiveIds: directives.map((d) => d.id) },
			provenance: {
				owner: "runtime",
				deterministic: true
			},
...
function validateAdherenceEvaluationPayload(raw, allowedDirectiveIds) {
	const entries = [];
	const verdicts = [];
	...
	if (!isAdherencePayload(raw)) {
		entries.push({
			...
			path: "payload",
			message: raw === void 0 || raw === null ? "No adherence evaluation payload was provided." : "Adherence evaluation payload must be an object with a verdicts array."
		});
		return {
			verdicts,
	...
	for (let i = 0; i < raw.verdicts.length; i++) {
		...
		if (!isVerdictEntry(item)) {
			entries.push({
				...
				directiveId: typeof item === "object" && item !== null ? item.directive_id : void 0
			});
			continue;
		}
...
function isVerdictEntry(value) {
	if (!value || typeof value !== "object" || Array.isArray(value)) return false;
	const entry = value;
	return typeof entry.directive_id === "string" && typeof entry.verdict === "string" && VALID_VERDICTS.has(entry.verdict) && typeof entry.confidence === "number" && typeof entry.reason === "string";
}
function buildAdherenceEvaluationSchema() {
	return {
		type: "object",
...
//#endregion
export { prepareAdherenceEvaluationContract, validateAdherenceEvaluationPayload };
