...
if (mode === 'ignore-term' || mode === 'ignore-term-no-output') {
  // Install the launcher handler before spawning the descendant so concurrent
  // test load cannot let the timeout race the fixture's intended force-kill
  // phase.
  process.on('SIGTERM', () => {});
}
